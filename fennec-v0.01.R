...
example_names 		= c("Bellis perennis","Brassica napus")
example_otutable 	= matrix(nrow=length(example_names),ncol=10,data=sample(c(rep(0,25),1:10),2*10,replace=T))
rownames(example_otutable) = example_names
example_traits 		= c("Plant Growth Habit","Leaf Area")

fennec4phyloseq <- function(mat){
		...
		for (j in 1:length(tmptraits$values)){
				...
				if (tmptraits$trait_format == "categorical_free"){
					...
					if (length(tmptraits$values[[names(tmptraits$values)[j]]])>0){
						value 	= as.character(tmptraits$values[[names(tmptraits$values)[j]]])
